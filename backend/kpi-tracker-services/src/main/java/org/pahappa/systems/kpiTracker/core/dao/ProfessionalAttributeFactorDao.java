package org.pahappa.systems.kpiTracker.core.dao;

import org.pahappa.systems.kpiTracker.models.ProfessionalAttributeFactor;
import org.sers.webutils.server.core.dao.BaseDao;

/**
 * Data Access Object for the {@link ProfessionalAttributeFactor} entity.
 */
public interface ProfessionalAttributeFactorDao extends BaseDao<ProfessionalAttributeFactor> {
}